...
def format_order_full_text(order_row: dict, *, include_comment: bool = False) -> str:
    """Текст заказа для баристы (позиции, сумма, время, клиент, при необходимости комментарий)."""
    ...
    items = parse_order_items(order_row)
    lines: list[str] = []
    for it in items:
        if it.get("kind") == "drink":
            size_key = str(it.get("size_key") or "")
            size_label = str(SIZES.get(size_key, {}).get("label", size_key))
            name = str(it.get("drink_name") or "?")
            price = int(it.get("price") or 0)
            lines.append(f"  • {name} ({size_label}) — {price} ₸")
        else:
