...
def kb_cart_delete(items: list[dict]) -> InlineKeyboardMarkup:
    """Кнопки удаления отдельных позиций из корзины."""
    rows: list[list[InlineKeyboardButton]] = []
    for it in items:
        label = str(it.get("label") or "?")[:48]
        rows.append(
            [InlineKeyboardButton(text=f"✕ {label}", callback_data=f"cart_del:{it['uid']}")]
        )
